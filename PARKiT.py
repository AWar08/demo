...
def extract_all_features(data, year='2021'):
    ...
    df['month'] = df['month'].cat.set_categories(new_categories=['Jan', 'Feb', 'Mar', 'Apr','May','Jun','Jul','Aug','Sep','Oct',
    'Nov','Dec'])
    ...
    df['day_of_week'] = df['day_of_week'].astype(pd.CategoricalDtype(categories=['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'], ordered=True))


    # Convert `hour` to categorical
    df['hour'] = df['hour'].astype('category') 
    df['hour'] = df['hour'].astype(pd.CategoricalDtype(categories=list(range(24)), ordered=True))
